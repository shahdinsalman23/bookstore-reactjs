...
const Products = ({ data }) => {
    // Default random products (dummy data)
    ...
    // Check if data exists, otherwise use random products
    const productsToShow = data.length ? data : randomProducts;
                    ...
                    <Col lg={3} md={4} sm={6} key={index}>
                        <div className='book-card'>
                            ...
                            <div className='card-detail'>
                                <h5>{e.volumeInfo.title}</h5>
                                <h6>Author: {e.volumeInfo.authors?.join(", ")}</h6>
                                <h4>${e.volumeInfo.pageCount}</h4>
                            </div>
                            <Link to={`/product/${index}`} className='theme_dark'>view detail</Link>
                        </div>
                    </Col>
                );
            })}
